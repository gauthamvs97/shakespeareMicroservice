...
def clean_Word(word):  # function that takes a word and returns only the alphabets in it. Ex: day's = days
    only_alpha = ""
    for char in word:
...
def count_Shakespeare(inputFile):
    ...
    for i in range(cur, total_Length - 1):
        if (i > 35 and len(lines[i]) > 0 and not lines[i] in chapters):
    ...
    for i in range(cur, total_Length - 1):
        if lines[i] in chapters:
            ...
            chapInfo["chapterName "] = chapterName
            chapInfo["noOfLines"] = c_Lines
            chapInfo["noOfUniqueWords"] = c_UniqueWords
            chapInfo["wordsByCharacter"] = word_with_Letter

            processingResult.append(chapInfo)

            # print("Unique words is ", c_UniqueWords)
            # print("Number of lines is ", c_Lines)
            # print(word_with_Letter, "\n")
            i = j
